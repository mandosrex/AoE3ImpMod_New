...
// initialize map type variables
  ...
  string baseMix = "korea_a";
  string baseTerrain = "water";
  string seaType = "Coastal Korea";
  ...
  string patchTerrain = "forest\ground_grass1_forest";
  string patchType1 = "forest\ground_grass1_forest";
  string patchType2 = "forest\ground_grass3_forest";
  
  string lightingType = "new england";
...
// Make one big island.  
   ...
   rmAddAreaInfluenceSegment(bigIslandID, 0.32, 1.0, 0.34, 0.19); 
   rmAddAreaInfluenceSegment(bigIslandID, 0.68, 1.0, 0.66, 0.19); 
...
// Set up player areas and starting locs
  ...
  if (cNumberTeams == 2)
  {
    ...
    else if (cNumberNonGaiaPlayers < 5)
    {
      ...
      rmPlacePlayersLine(0.69, 0.4, 0.69, 0.6, 0.1, 0);                
      ...
      rmPlacePlayersLine(0.31, 0.6, 0.31, 0.4, 0.1, 0); 
    }  
    else
    {
      if (teamStartLoc > 0.5)
      ...
      rmPlacePlayersLine(0.7, 0.35, 0.7, 0.65, 0.1, 0);                
      ...
      rmPlacePlayersLine(0.3, 0.65, 0.3, 0.35, 0.1, 0); 
    } 
  }
  else  // otherwise FFA
  {
...
//Prepare to place player starting trees
	...
	rmSetObjectDefMaxDistance(StartAreaTreeID, 22.0);	
...
// Cliffs
   ...
   for (i=0; <numCliffs)
   {
	...
	rmSetAreaCliffHeight(bigCliffID, cliffHt, 1.0, 0.7);
...
// Huntables - near and far
   ...
   rmAddObjectDefItem(turkeyID, huntable1, rmRandInt(4,6), 5.0);
