 ...
 */
   ...
   // Map edge constraints
	...
	int shortAvoidStartingUnits2=rmCreateClassDistanceConstraint("objects avoid starting units short 2", rmClassID("startingUnit"), 15.0);
...
if (seasonPicker < 0.5)
	...
	//if (seasonPicker < 0.5)
		rmSetAreaTerrainType(tradeIslandA, "new_england\ground2_ne");
	//else
...
// *********************************** PLACE PLAYERS ************************************
	...
	if ( cNumberTeams <= 2 && teamZeroCount <= 4 && teamOneCount <= 4)
	{
		...
		rmSetPlacementSection(0.6, 0.8); // 0.5
		rmSetTeamSpacingModifier(0.25);
		rmPlacePlayersCircular(0.30, 0.32, 0);
			
		rmSetPlacementTeam(1);
		rmSetPlacementSection(0.1, 0.3); // 0.5
		rmSetTeamSpacingModifier(0.25);
		rmPlacePlayersCircular(0.30, 0.32, 0);
	}
	else
...
//		rmAddAreaConstraint(id, avoidWater);
  ...
  int berry = 0;
	...
	if (seasonPicker < 0.5){
		...
		rmAddObjectDefItem(StartElkID, "moose", 2, 4.0);
  }
	else
	{
		rmAddObjectDefItem(StartElkID, "berrybush", 2, 4.0);
		rmAddObjectDefItem(StartElkID, "moose", 4, 4.0);
    berry = 1;
	...
	for (i=0; < 50)
      {
		 ...
		 if (seasonPicker < 0.5)
			rmSetAreaTerrainType(patch, "new_england\ground2_ne");
		 else
...
*/
   ...
   rmSetStatusText("",0.30);
	...
	{  
		...
		rmAddGroupingConstraint(cheyenneVillageID, shortAvoidStartingUnits2);
	...
	if ( cNumberNonGaiaPlayers >= 4 )
	{
		if (subCiv1 == rmGetCivID("huron"))
		{
			...
			rmAddGroupingConstraint(huron2VillageID, shortAvoidStartingUnits2);
		...
		else
		{
			...
			rmAddGroupingConstraint(cree2VillageID, shortAvoidStartingUnits2);
		...
		if (subCiv2 == rmGetCivID("huron"))
		{
			...
			rmAddGroupingConstraint(huron3VillageID, shortAvoidStartingUnits2);
		...
		else
		{
			...
			rmAddGroupingConstraint(cree3VillageID, shortAvoidStartingUnits2);
	...
	if (subCiv3 == rmGetCivID("Cree"))
	{
		...
		rmAddGroupingConstraint(cree4VillageID, shortAvoidStartingUnits2);
	...
	{   
		...
		rmAddGroupingConstraint(huron4VillageID, shortAvoidStartingUnits2);
	...
	{   
		...
		rmAddGroupingConstraint(cheyenne2VillageID, shortAvoidStartingUnits2);
...
// Place some extra deer herds.  
	...
	rmAddObjectDefItem(mooseHerdID, "moose", rmRandInt(3,4), 6.0);
