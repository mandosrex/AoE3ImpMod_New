...
// revised for better resource balance by RF_Gandalf
// Update by iCourt
...
void main(void)
{
   ...
   int Eastward=rmCreatePieConstraint("eastMapConstraint", 0.53, 0.5, 0, rmZFractionToMeters(0.5), rmDegreesToRadians(0), rmDegreesToRadians(180));
   int Westward=rmCreatePieConstraint("westMapConstraint", 0.47, 0.5, 0, rmZFractionToMeters(0.5), rmDegreesToRadians(180), rmDegreesToRadians(0));
	...
	int extremeSEconstraint = rmCreateBoxConstraint("stay deep into SE portion", 0, 0, 1, 0.4);
 	int extremeNconstraint = rmCreateBoxConstraint("stay deep into N portion", 0, 0.6, 1, 1);
 	int centerConstraint = rmCreateBoxConstraint("stay in center portion", 0, 0.5, 1, 0.58);
  
	// Player constraints
   int playerConstraint=rmCreateClassDistanceConstraint("stay away from players", classPlayer, 20.0);
   int medPlayerConstraint=rmCreateClassDistanceConstraint("stay away from players med", classPlayer, 32.0);
   int farPlayerConstraint=rmCreateClassDistanceConstraint("stay away from players far", classPlayer, 60.0);
   ...
   if (cNumberNonGaiaPlayers > 6)
      avoidCoin=rmCreateTypeDistanceConstraint("avoid coin shorter", "gold", 25.0);
   ...
   rmSetAreaTerrainType(southIslandID, "yukon\ground7_yuk");
   rmAddAreaTerrainLayer(southIslandID, "yukon\ground6_yuk", 0, 3);
   rmAddAreaTerrainLayer(southIslandID, "yukon\ground5_yuk", 3, 6);
   rmAddAreaTerrainLayer(southIslandID, "yukon\ground7_yuk", 6, 8);
   ...
   for(i=1; <cNumberPlayers)
   {
      ...
      rmSetAreaLocPlayer(id, i);
		 rmSetAreaTerrainType(id, "yukon\ground7_yuk");
   ...
   rmBuildAllAreas();
	...
	rmAddObjectDefItem(StartCaribouID, "caribou", 6, 4.0);
      rmAddObjectDefToClass(StartCaribouID, rmClassID("huntableFood"));
	rmSetObjectDefMinDistance(StartCaribouID, 11.0);
	rmSetObjectDefMaxDistance(StartCaribouID, 14.0);
	rmSetObjectDefCreateHerd(StartCaribouID, false);
	rmAddObjectDefConstraint(StartCaribouID, avoidStartingUnitsSmall);

	int StartCaribouID2=rmCreateObjectDef("starting caribou 2");
	rmAddObjectDefItem(StartCaribouID2, "caribou", 6, 4.0);
      rmAddObjectDefToClass(StartCaribouID2, rmClassID("huntableFood"));
	...
	for(i=1; <cNumberPlayers)
	{
		...
		// Everyone gets one close ore grouping.
		...
		rmPlaceObjectDefAtLoc(StartCaribouID, 0, rmXMetersToFraction(xsVectorGetX(TCLocation)), rmZMetersToFraction(xsVectorGetZ(TCLocation)));
		rmPlaceObjectDefAtLoc(StartCaribouID2, 0, rmXMetersToFraction(xsVectorGetX(TCLocation)), rmZMetersToFraction(xsVectorGetZ(TCLocation)));
		...
		// Everyone gets one far ore grouping.
		silverType = rmRandInt(1,10);
		playerGold2ID = rmCreateObjectDef("player silver farther "+i);
		rmAddObjectDefItem(playerGold2ID, "mine", 1, 0.0);			
		rmAddObjectDefToClass(playerGold2ID, rmClassID("importantItem"));
		rmSetObjectDefMinDistance(playerGold2ID, 38.0);
		rmSetObjectDefMaxDistance(playerGold2ID, 40.0);
		rmAddObjectDefConstraint(playerGold2ID, avoidAll);
		rmAddObjectDefConstraint(playerGold2ID, avoidCoin);
		rmAddObjectDefConstraint(playerGold2ID, medPlayerConstraint);
		rmAddObjectDefConstraint(playerGold2ID, avoidImpassableLand);
		rmAddObjectDefConstraint(playerGold2ID, avoidTradeRouteForest);
...
// Natives
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaForestDensity(snowforestID, 0.6);
			rmSetAreaForestClumpiness(snowforestID, 0.3);		
			...
			rmAddAreaConstraint(snowforestID, avoidAll);
	...
	for(i=0; < silverCount2)
	{
		...
		rmAddObjectDefConstraint(silverID, avoidTradeRouteNugget);
		...
		rmAddObjectDefConstraint(silverID, extremeNconstraint);
		rmPlaceObjectDefAtLoc(silverID, 0, 0.5, 0.7);
   }
   ...
   */
	...
	for(i=0; < silverCount1)
	{
		...
		rmAddObjectDefConstraint(silverID, Southward);
		rmAddObjectDefConstraint(silverID, avoidWater12);
	...
	for(i=0; < silverCount1)
	{
		...
		rmAddObjectDefConstraint(silverID, Southward);
		rmAddObjectDefConstraint(silverID, avoidWater12);
		rmPlaceObjectDefAtLoc(silverID, 0, 0.4, 0.4);
   }
...
*/
	...
	int muskOxEID=rmCreateObjectDef("east muskOx herd");
	rmAddObjectDefItem(muskOxEID, "muskOx", rmRandInt(8,10), 8.0);
      rmAddObjectDefToClass(muskOxEID, rmClassID("huntableFood"));
	rmSetObjectDefMinDistance(muskOxEID, 0.0);
	rmSetObjectDefMaxDistance(muskOxEID, rmXFractionToMeters(0.4));
	rmAddObjectDefConstraint(muskOxEID, avoidMuskOx);
	rmAddObjectDefConstraint(muskOxEID, avoidAll);
	rmAddObjectDefConstraint(muskOxEID, centerConstraint);
	rmAddObjectDefConstraint(muskOxEID, avoidImpassableLand);
	rmAddObjectDefConstraint(muskOxEID, Eastward);
	rmAddObjectDefConstraint(muskOxEID, medPlayerConstraint);
      rmAddObjectDefConstraint(muskOxEID, huntableConstraint);
	rmSetObjectDefCreateHerd(muskOxEID, true);

	int muskOxWID=rmCreateObjectDef("west muskOx herd");
	rmAddObjectDefItem(muskOxWID, "muskOx", rmRandInt(8,10), 8.0);
      rmAddObjectDefToClass(muskOxWID, rmClassID("huntableFood"));
	rmSetObjectDefMinDistance(muskOxWID, 0.0);
	rmSetObjectDefMaxDistance(muskOxWID, rmXFractionToMeters(0.4));
	rmAddObjectDefConstraint(muskOxWID, avoidMuskOx);
	rmAddObjectDefConstraint(muskOxWID, avoidAll);
	rmAddObjectDefConstraint(muskOxWID, centerConstraint);
	rmAddObjectDefConstraint(muskOxWID, avoidImpassableLand);
	rmAddObjectDefConstraint(muskOxWID, Westward);
	rmAddObjectDefConstraint(muskOxWID, medPlayerConstraint);
      rmAddObjectDefConstraint(muskOxWID, huntableConstraint);
	rmSetObjectDefCreateHerd(muskOxWID, true);

	int caribouID=rmCreateObjectDef("east caribou herd");
	rmAddObjectDefItem(caribouID, "caribou", rmRandInt(8,12), 8);
      rmAddObjectDefToClass(caribouID, rmClassID("huntableFood"));
	rmSetObjectDefMinDistance(caribouID, 0.0);
	rmSetObjectDefMaxDistance(caribouID, rmXFractionToMeters(0.4));
	...
	rmAddObjectDefConstraint(caribouID, Eastward);
	rmAddObjectDefConstraint(caribouID, medPlayerConstraint);
      rmAddObjectDefConstraint(caribouID, huntableConstraint);
	rmSetObjectDefCreateHerd(caribouID, true);

	int caribouWID=rmCreateObjectDef("west caribou herd");
	rmAddObjectDefItem(caribouWID, "caribou", rmRandInt(8,12), 8);
      rmAddObjectDefToClass(caribouWID, rmClassID("huntableFood"));
	rmSetObjectDefMinDistance(caribouWID, 0.0);
	rmSetObjectDefMaxDistance(caribouWID, rmXFractionToMeters(0.4));
	rmAddObjectDefConstraint(caribouWID, avoidCaribou);
	rmAddObjectDefConstraint(caribouWID, avoidAll);
	rmAddObjectDefConstraint(caribouWID, SEconstraint);
	rmAddObjectDefConstraint(caribouWID, avoidImpassableLand);
	rmAddObjectDefConstraint(caribouWID, Westward);
	rmAddObjectDefConstraint(caribouWID, medPlayerConstraint);
      rmAddObjectDefConstraint(caribouWID, huntableConstraint);
	rmSetObjectDefCreateHerd(caribouWID, true);

	rmPlaceObjectDefAtLoc(muskOxEID, 0, 0.65, 0.5, cNumberNonGaiaPlayers/2);
	rmPlaceObjectDefAtLoc(muskOxWID, 0, 0.35, 0.5, cNumberNonGaiaPlayers/2);

	rmPlaceObjectDefAtLoc(caribouWID, 0, 0.4, 0.4, cNumberNonGaiaPlayers);
	rmPlaceObjectDefAtLoc(caribouID, 0, 0.6, 0.4, cNumberNonGaiaPlayers);

	int muskOxID=rmCreateObjectDef("north muskOx herd");
	rmAddObjectDefItem(muskOxID, "muskOx", rmRandInt(8,10), 8.0);
      rmAddObjectDefToClass(muskOxID, rmClassID("huntableFood"));
	...
	rmAddObjectDefConstraint(muskOxID, extremeNconstraint);
	rmAddObjectDefConstraint(muskOxID, avoidImpassableLand);
	rmAddObjectDefConstraint(muskOxID, medPlayerConstraint);
      rmAddObjectDefConstraint(muskOxID, huntableConstraint);
	rmSetObjectDefCreateHerd(muskOxID, true);

	rmPlaceObjectDefAtLoc(muskOxID, 0, 0.5, 0.7, (cNumberNonGaiaPlayers+2));
