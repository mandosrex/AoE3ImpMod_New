...
int sfBuildBigIsland (string bigIslandName = "", float x_loc = 0.0, float y_loc = 0.0, float mainlandType = 0.0)
{
  ...
  rmSetAreaCoherence(bigIslandID, 0.6);
  ...
  if(mainlandType == 1.0) {
    ...
    rmSetObjectDefMinDistance(socketID, 20.0);
    rmSetObjectDefMaxDistance(socketID, 80.0);
    ...
    if(x_loc+y_loc > 1.0) {
      rmAddTradeRouteWaypoint(tradeRouteID, 1.0, 0.47);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.8, 0.6);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.6, 0.8);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.47, 1.0);
      rmEchoInfo("Up top");
    }
    
    else {
      rmAddTradeRouteWaypoint(tradeRouteID, 0.53, 0.0);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.4, 0.19);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.19, 0.4);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.0, 0.53);
...
void main(void)
{
  ...
  int avoidBanner = rmCreateTypeDistanceConstraint("versus banners", "ypPropsJapaneseBanner", 75.0);
	...
	avoidWater20 = rmCreateTerrainDistanceConstraint("avoid water long", "Land", false, 25.0);
