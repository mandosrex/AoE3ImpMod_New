...
void main(void)
{
  ...
  // Text
   ...
   int nearWater15 = rmCreateTerrainMaxDistanceConstraint("near water 2", "Water", true, 15.0);
  ...
  int avoidTCMedium=rmCreateTypeDistanceConstraint("stay away from TC by a bit", "TownCenter", 18.0);
  ...
  float yLoc = 0.0;
   ...
   for(i=0; <cNumberTeams)
   {
      ...
      rmSetAreaMaxBlobDistance(teamID, 40.0);
		rmSetAreaCoherence(teamID, 0.65);
...
//	rmAddAreaConstraint(teamID, playerEdgeConstraint);
   ...
   rmClearClosestPointConstraints(); 
	...
	rmSetObjectDefMaxDistance(socketID, 90.0);
	rmAddObjectDefConstraint(socketID, socketAvoidSocket);
	rmAddObjectDefConstraint(socketID, avoidTeamIslands);
	rmAddObjectDefConstraint(socketID, avoidWater5);
	rmAddObjectDefConstraint(socketID, nearWater15);

	int tradeRoute1ID = rmCreateTradeRoute();

	if (IslandLoc == 1)
	{
		rmAddTradeRouteWaypoint(tradeRoute1ID, 1.0, 0.5);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.63, 0.63);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.5, 1.0);
	}
	else if (IslandLoc == 2)
	{
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.5, 0.0);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.63, 0.37);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 1.0, 0.5);
	}
	else if (IslandLoc == 3)
	{
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.5, 0.0);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.37, 0.37);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.0, 0.5);
	}
	else
	{
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.0, 0.5);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.37, 0.63);
		rmAddTradeRouteWaypoint(tradeRoute1ID, 0.5, 1.0);
	}		

	bool placedTradeRoute1 = rmBuildTradeRoute(tradeRoute1ID, "naval");
   ...
   rmSetObjectDefTradeRouteID(socketID, tradeRoute1ID);
	...
	socketLoc1 = rmGetTradeRouteWayPoint(tradeRoute1ID, 0.8);
	rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);
  ...
  if (cNumberTeams == 2)
	...
	rmAddObjectDefConstraint(TCID, avoidTCMedium);
  ...
  rmAddObjectDefItem(playerDeerID, "tortoise", rmRandInt(15,15), 10.0);
  rmSetObjectDefMinDistance(playerDeerID, 15.0);
  rmSetObjectDefMaxDistance(playerDeerID, 20.0);
  ...
  rmSetObjectDefCreateHerd(playerDeerID, false);
    ...
    if ( rmGetNomadStart())
    {
        rmAddObjectDefItem(colonyShipID, "Privateer", 1, 10.0);
    }
  ...
  for (i=0; <numTries) {   
    ...
    rmAddAreaConstraint(forest, avoidTCshort);
 ...
 // Define and place Nuggets
  ...
  if(rmGetIsKOTH()) {
   ...
   if(bonusChance<0.5)   
      rmAddObjectDefItem(deerID, "tortoise", rmRandInt(5,7), 10.0);
   else
      rmAddObjectDefItem(deerID, "crab", rmRandInt(8,10), 10.0);
