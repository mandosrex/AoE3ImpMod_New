...
// initialize map type variables
  ...
  int avoidTradeRouteSocketsLong = 0;
...
int sfBuildBigIsland (string bigIslandName = "", float x_loc = 0.0, float y_loc = 0.0, float mainlandType = 0.0)
{
  ...
  if(mainlandType == 1.0) {
    ...
    rmSetObjectDefMaxDistance(socketID, 70.0);
    rmAddObjectDefConstraint(socketID, avoidImpassableLand);
    rmAddObjectDefConstraint(socketID, avoidTradeRouteSocketsLong);
    ...
    if(x_loc+y_loc > 1.0) {
      ...
      rmAddTradeRouteWaypoint(tradeRouteID, 0.8, 0.6);
      ...
      rmAddTradeRouteWaypoint(tradeRouteID, 0.34, 0.2);
      rmAddTradeRouteWaypoint(tradeRouteID, 0.2, 0.34);
...
void main(void)
{
  ...
  avoidImpassableLandLong=rmCreateTerrainDistanceConstraint("avoid impassable land long", "Land", false, 15.0);
	...
	avoidWater20 = rmCreateTerrainDistanceConstraint("avoid water long", "Land", false, 12.0);
	avoidWater8 = rmCreateTerrainDistanceConstraint("avoid water 8", "Land", false, 8.0);
	hugWater = rmCreateTerrainMaxDistanceConstraint("hug water 2", "water", true, 15.0);
  ...
  avoidTradeRouteSocketsNear=rmCreateTypeDistanceConstraint("avoid trade route sockets near", "sockettraderoute", 5.0);
	avoidTradeRouteSocketsLong=rmCreateTypeDistanceConstraint("avoid trade route sockets long", "sockettraderoute", 40.0);
  ...
  rmEchoInfo("30 percent map loaded");
	...
	// Make one big island.  
	int bigIsland2ID=rmCreateArea("big lone island");
	rmSetAreaSize(bigIsland2ID, 0.17, 0.17);
	rmSetAreaCoherence(bigIsland2ID, 0.7);
	rmSetAreaBaseHeight(bigIsland2ID, 2.0);
	rmSetAreaSmoothDistance(bigIsland2ID, 20);
	rmSetAreaMix(bigIsland2ID, baseMix);
	rmAddAreaToClass(bigIsland2ID, classIsland);
	rmAddAreaConstraint(bigIsland2ID, islandConstraint);
	rmSetAreaObeyWorldCircleConstraint(bigIsland2ID, false);
	rmSetAreaElevationType(bigIsland2ID, cElevTurbulence);
	rmSetAreaElevationVariation(bigIsland2ID, 4.0);
	rmSetAreaElevationMinFrequency(bigIsland2ID, 0.09);
	rmSetAreaElevationOctaves(bigIsland2ID, 3);
	rmSetAreaElevationPersistence(bigIsland2ID, 0.2);
	rmSetAreaElevationNoiseBias(bigIsland2ID, 1);

  rmAddAreaInfluenceSegment(bigIsland2ID, 0.30, 0.64, 0.30, 0.71);
  rmAddAreaInfluenceSegment(bigIsland2ID, 0.34, 0.60, 0.33, 0.65);
  rmAddAreaInfluenceSegment(bigIsland2ID, 0.46, 0.525, 0.48, 0.48);
  rmAddAreaInfluenceSegment(bigIsland2ID, 0.50, 0.475, 0.525, 0.45);
  rmAddAreaInfluenceSegment(bigIsland2ID, 0.59, 0.34, 0.67, 0.33);
  rmAddAreaInfluenceSegment(bigIsland2ID, 0.63, 0.31, 0.71, 0.28);    

	// --------------- Make load bar move. ----------------------------------------------------------------------------
	rmSetStatusText("",0.40);
    rmEchoInfo("40 percent map loaded");

	rmSetAreaWarnFailure(bigIsland2ID, false);

	rmSetAreaLocation(bigIsland2ID, .5, .5);		//Put the big island in exact middle of map.
	rmBuildArea(bigIsland2ID);
  ...
  if (cNumberTeams == 2 ) {
    ...
    int id=rmCreateArea("Player"+i, bigIsland2ID);
  ...
  {  
    ...
    rmPlaceGroupingInArea(caribs5VillageID, 0, bigIsland2ID, 1);
  }	
  ...
  {  
    ...
    rmPlaceGroupingInArea(caribs6VillageID, 0, bigIsland2ID, 1);
  } 
  ...
  rmAddObjectDefConstraint(playerGoldID, avoidBerries);
	...
	rmSetObjectDefMinDistance(playerCrateID, 10.0);
  ...
  rmSetObjectDefMinDistance(playerPopBlock, 8);
  rmSetObjectDefMaxDistance(playerPopBlock, 11);
  ...
  rmAddObjectDefConstraint(goldID, avoidImportantItem);    
	rmPlaceObjectDefInArea(goldID, 0, bigIsland2ID, cNumberNonGaiaPlayers*2.5);
  ...
  rmAddObjectDefConstraint(berriesID, avoidImportantItem);    
	rmPlaceObjectDefInArea(berriesID, 0, bigIsland2ID, cNumberNonGaiaPlayers*2); 
  ...
  for (i=0; <numTries) {   
    int forest=rmCreateArea("forest "+i, bigIsland2ID);
    rmSetAreaWarnFailure(forest, false);
	...
	rmAddObjectDefItem(turkeyID, huntable1, rmRandInt(4,5), 5.0);
  ...
  rmSetObjectDefCreateHerd(turkeyID, true);
	rmPlaceObjectDefInArea(turkeyID, 0, bigIsland2ID, cNumberNonGaiaPlayers*2);
  ...
  rmAddObjectDefConstraint(nugget1, avoidImportantItem); 
	...
	rmPlaceObjectDefInArea(nugget1, 0, bigIsland2ID, cNumberNonGaiaPlayers*3);
  
  if(cNumberTeams == 2 && cNumberNonGaiaPlayers > 2){
    rmSetNuggetDifficulty(3, 4);
	  rmPlaceObjectDefInArea(nugget1, 0, bigIsland2ID, cNumberNonGaiaPlayers);
  }
	// Water nuggets
