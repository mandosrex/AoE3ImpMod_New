...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetBaseTerrainMix("rockies_grass"); // nwt_grass1
	rmTerrainInitialize("great_lakes\ground_grass1_gl", 4.0);
	...
	rmSetAreaTerrainType(groundID, "great_lakes\ground_grass1_gl");
...
//	rmAddAreaConstraint(groundID, avoidCliff);
	...
	rmSetAreaWaterType(Lake1ID, "rockies water"); //yukon river
...
//	rmSetAreaMaxBlobDistance(Lake1ID, 20);
	...
	rmSetAreaWaterType(Lake2ID, "rockies water"); //yukon river
...
//	rmSetAreaMaxBlobDistance(Lake2ID, 20);
	...
	rmSetAreaWaterType(Lake3ID, "rockies water"); //yukon river
...
//	rmSetAreaCliffType(plateau3ID, "rocky mountain2"); // araucania north coast
...
//	rmSetAreaTerrainType(snowrim3ID, "araucania\ground_snow3_ara");
	...
	for(i=1; < 3)
	{
		...
		rmSetAreaCliffType(borderID, "rocky mountain2"); //new england snow, rocky mountain edge
	//	rmSetAreaCliffPainting(borderID, false, true, true, true); //  paintGround,  paintOutsideEdge,  paintSide,  minSideHeight,  paintInsideEdge
		...
		rmSetAreaCliffHeight(borderID, 8, 0.0, 0.2); 
	...
	rmSetAreaCliffType(border3ID, "rocky mountain2"); //new england snow, rocky mountain edge
//	rmSetAreaCliffPainting(border3ID, false, true, true, true); //  paintGround,  paintOutsideEdge,  paintSide,  minSideHeight,  paintInsideEdge
	...
	rmSetAreaCliffHeight(border3ID, 8, 0.0, 0.2); 
	...
	for (i=0; < 12*cNumberNonGaiaPlayers)
    {
		...
		rmSetAreaMix(patch1ID, "flowers_grass");
...
//		rmAddAreaConstraint(patch1ID, avoidBorder);
	...
	for (i=0; < 12*cNumberNonGaiaPlayers)
    {
		...
		rmSetAreaMix(patch2ID, "flowers_grass");
...
//		rmSetAreaTerrainType(plateauforestID, "pampas\groundforest_pam");
		rmSetAreaTerrainType(plateauforestID, "great_lakes\ground_grass3_gl");
	...
	for (i=0; < valleyforestcount)
	{
		...
		rmSetAreaTerrainType(valleyforestID, "great_lakes\ground_grass6_gl");
	...
	for(i=0; < herdcount)
	{
		...
		rmAddObjectDefItem(elkherdID, "elk", rmRandInt(6,7), 5.0);
	...
	for (i=0; < 5+cNumberNonGaiaPlayers/2)
	{
		if (cNumberTeams <= 2)
		{
