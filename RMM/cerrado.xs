...
for(i=1; < cNumberNonGaiaPlayers + 1) {
	...
	rmAddObjectDefConstraint(playerStart, avoidWaterShort);
        ...
        rmAddObjectDefItem(treeID, "TreePuya", rmRandInt(6,7), 7.0);
...
rmAddObjectDefConstraint(playerNuggetID, avoidCoin);
  ...
  rmSetGroupingMaxDistance(playerWalls, 3.0);
    ...
    for(i=1; < cNumberNonGaiaPlayers + 1) {
		...
		if(rmGetNomadStart()){
			rmAddObjectDefItem(startID2, "CoveredWagon", 1, 2.0);
		}else{
...
//==========hills==============
	...
	//for (j=0; < (6*cNumberNonGaiaPlayers)) {   
		...
		rmAddObjectDefItem(mapTrees, "TreePolyepis", rmRandInt(2,2), rmRandFloat(5.0,6.0));
        rmAddObjectDefItem(mapTrees, "TreePuya", rmRandInt(1,2), 4.0);
