...
// ********************* MAP PARAMETERS ************************
  ...
  int avoidHill = rmCreateTypeDistanceConstraint("avoid hill", "ypKingsHill", 8.0);
	...
	int StartTurkeyID=rmCreateObjectDef("starting hunts");
	rmAddObjectDefItem(StartTurkeyID, "Pronghorn", 4, 4.0);
...
//**************************************** cliff embellishments *********************************************
	for(i=0; <numTries)
	{
		...
		if (cliffHeight <= 5)
		rmSetAreaCliffHeight(mesaID, rmRandInt(4,8), 1.0, 1.0);
		else
		rmSetAreaCliffHeight(mesaID, -9, 1.0, 1.0);
