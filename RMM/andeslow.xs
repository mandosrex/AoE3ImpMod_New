...
void main(void)
{
   ...
   // Nature avoidance
	...
	int avoidDeer=rmCreateTypeDistanceConstraint("deer avoids deer", "Guanaco", 50.0);
   ...
   rmSetStatusText("",0.10);
	...
	{
		rmSetPlacementSection(0.50, 1.00);
...
*/	
		...
		if(rmRandFloat(0,1) > 0.50)
		{
			...
			rmPlacePlayersLine(0.20, 0.90, 0.60, 1.00, 0.2, 0.1);

			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.20, 0.10, 0.60, 0.00, 0.2, 0.1);
		}
		else
		{
			rmSetPlacementTeam(0);
   ...
   rmBuildAllAreas();
	...
	int tradeRoute1ID = rmCreateTradeRoute();
		...
		else
		{
			...
			rmRiverAddWaypoint(river3ID, 0.85, 0.0);
			rmRiverAddWaypoint(river3ID, 0.55, 0.3);
			rmRiverAddWaypoint(river3ID, 0.0, 0.2);
			rmRiverSetShallowRadius(river3ID, 10);
			rmRiverAddShallow(river3ID, rmRandFloat(0.5, 0.6));
...
*/
	...
	for (i=0; <numTries)
		{   
			...
			rmAddAreaConstraint(forestnorthID, shortAvoidImpassableLand);
	...
	if(rmRandFloat(0,1) < 0.6) //place llamas 60% of the time
		...
		for (i=0; <llamaCount)
			...
			rmAddObjectDefConstraint(llamaID, shortAvoidImpassableLand);
	...
	for (i=0; <guanacoCount/2)
	{
		...
		rmAddObjectDefConstraint(guanacoID, shortAvoidImpassableLand);
	...
	for (i=0; <guanacoCount/2)
	{
		...
		rmAddObjectDefConstraint(guanacoSEID, shortAvoidImpassableLand);
